...
void* sort_2(void* args){
    ...
    shuffle((void*) &(someargs){.v = arg->v, .l = arg->l, .r = arg->r, .tmp = arg->tmp});
    int m = (arg->l + arg->r + 1) / 2;
    sort_2((void*) &(someargs){.v = arg->v, .l = arg->l, .r = m - 1, .tmp = arg->tmp});
    sort_2((void*) &(someargs){.v = arg->v, .l = m, .r = arg->r, .tmp = arg->tmp});
    unshuffle((void*) &(someargs){.v = arg->v, .l = arg->l, .r = arg->r, .tmp = arg->tmp});
...
void sort_1(vector* v,int maxcount){
    ...
    for(int j = 1; j <= lenth_vector(v); j = j * 2 + 1){
        for(int i = 0; i <= lenth_vector(v) - j - 1; i = 1 + j + i){
            ...
            if(fl){
                shuffle((void*) &(someargs){.v = v, .l = i, .r = i + j, .tmp = &tmp});
                int m = (i + i + j + 1) / 2;
                z++;
            ...
            if(!fl){
                sort_2((void*) &(someargs){.v = v, .l = i, .r = i + j, .tmp = &tmp});
            }
        }
        if(fl){
            for(int i = z0; i <= z; i++){
            ...
            for(int i = i0; i <= lenth_vector(v) - j - 1; i = 1 + j + i){
                unshuffle((void*) &(someargs){.v = v, .l = i, .r = i + j, .tmp = &tmp});
                for(int k = i ; k < i + j; k++){
                    cmp(&v->d[k], &v->d[k + 1]);
               }
            }
